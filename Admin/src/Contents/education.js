...
const Education = () => {
  ...
  const getData = () => {
    getEducation('/Education/GetAllEducation').then((response) => setData(response.data.data))
    .catch((error) => console.error(error));
  }
  // fromDate: formatDate(currentItem.fromDate),
  // toDate: formatDate(currentItem.toDate),
  const handleAddOrUpdate = () => {
    ...
    const url = '/Education/AddEducation'
    const url1 ='/Education/UpdateEducation'
    if(isEditing)
  ...
  const handleDelete = (id) => {
      deleteEducation(`/Education/DeleteEducation?Id=${id}`).then((res)=>{
        if(res.data.isSuccess){
       console.log(res.data.message);
       getData();
        }
